...
const form = useForm({
    ...
    mobile_no: props.factory.mobile_no || '',
    mobile_no_two: props.factory.mobile_no_two || '',
});

const openEditModal = () => {
    form.name = props.factory.name;
    form.address = props.factory.address || '';
    form.mobile_no = props.factory.mobile_no || '';
    form.mobile_no_two = props.factory.mobile_no_two || '';
    form.clearErrors();
...
const submitForm = () => {
    form.put(route('factory.update', props.factory.id), {
        onSuccess: () => {
            closeModal();
        }
    });
...
const deleteFactory = () => {
    if (confirm(t('pages.Are you sure?'))) {
        router.delete(route('factory.destroy', props.factory.id));
    }
};
</script>
