...
class PurchaseEntry extends Model
{
    ...
    protected static function getPurchasesOnQuery($date) {
        ...
        return self::join('inventories', 'inventories.shoe_id', '=', 'purchase_entries.shoe_id')
            ->where('purchase_entries.created_at', '>=', $current)
            ->where('purchase_entries.created_at', '<', $next)
            ->groupBy('purchase_entries.purchase_id','inventories.factory', 'inventories.retail_price', 'inventories.category')
            ->selectRaw('purchase_entries.purchase_id, inventories.factory, inventories.category, group_concat(distinct inventories.color separator "+") color, inventories.retail_price, group_concat(distinct inventories.purchase_price separator "/") purchase_price, sum(purchase_entries.count) count, sum(purchase_entries.count * inventories.purchase_price / 12) total_price_a');
    }
    ...
    {
        static::deleting(function ($purchaseEntry) {
            if (auth()->check()) {
                $purchaseEntry->deleted_by = auth()->id();
                $purchaseEntry->save();
            }
        });
    }

    protected $fillable = ['shoe_id', 'count','purchase_price','retail_price'];
