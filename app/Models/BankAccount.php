...
class BankAccount extends Model
{
    ...
    {
        $final_balance = $this->getCurrentAccountBook()->getOpeningBalance() ?? 0;
        $desired_balances = [];
        foreach ($this->entries->reverse() as $entry) {
            if (in_array($entry->type, ['deposit', 'income'])) {
    ...
    {
        return $this->account_no === 'cash'
            ? $this->bank
            : $this->bank . ', ' . $this->branch . ' শাখা (অ্যাকাউন্ট নং ' . $this->account_no . ')';
    }
